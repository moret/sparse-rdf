...
# slightly modified by moret to ignore bad lines and log
...
class NTriplesParser(object):
   ...
   def parse(self, f):
      ...
      while True:
         ...
         line_num += 1
         if line_num % 100000 == 0: print '%s: lines so far: %d' % (datetime.now().isoformat(), line_num)
         if self.line is None: break
         try: self.parseline()
         except ParseError:
            # raise ParseError("Invalid line: %r" % self.line)
            print "Invalid line (%d): %r" % (line_num, self.line)
      return self.sink
